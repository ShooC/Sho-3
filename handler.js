                       ...
                       let pp = 'https://telegra.ph/file/2e77ffa63ce0b7f8c5163.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Yah,si Beban Masuk Grup @user').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                ...
                                this.send2ButtonImg(id, pp, text, wm3, "Menu Bot", ".menu", "Intro for new memb", "intro", null)
                                }
                    }
                }
                break
...
`.trim(), wm, 'Owner', '.owner', 'Menu', '.menu', m.message, {
  ...
  async GroupUpdate({ jid, desc, descId, descTime, descOwner, announce }) {
    ...
    ketik *.off desc* untuk mematikan pesan ini
        `.trim()
    this.send2But(jid, caption, wm, 'Owner', '.owner', 'Menu', '.menu', { contextInfo: { mentionedJid: this.parseMention(caption) } })
...
let name = conn.getName(m.sender)
 //let ppacc = global.ppacc
  ...
  let ac = {
      ...
      owner: `Hai kak 👋, @${m.sender.split`@`[0]}`,    
      mods: `Hai kak 👋, @${m.sender.split`@`[0]}`,     
      premium: `Hai kak 👋, @${m.sender.split`@`[0]}`,      
      group: `Hai kak 👋, @${m.sender.split`@`[0]}`,      
      private: `Hai kak 👋, @${m.sender.split`@`[0]}`,      
      admin: `Hai kak 👋, @${m.sender.split`@`[0]}`,      
      botadmin: `Hai kak 👋, @${m.sender.split`@`[0]}`,  
      }[type]
  if (msg) return conn.send2But(m.chat, ac, msg, 'Owner', '.owner', 'Menu', '.menu', m)
